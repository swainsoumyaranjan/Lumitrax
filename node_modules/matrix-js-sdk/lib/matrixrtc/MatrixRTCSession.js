  ...
  static callMembershipsForRoom(room) {
    ...
    for (const memberEvent of callMemberEvents) {
      ...
      if (eventMemberships === undefined) {
        _logger.logger.warn(`Ignoring malformed member event from ${memberEvent.getSender()}: no memberships section`);
        continue;
      }
      ...
      for (const membershipData of eventMemberships) {
        try {
          ...
          if (membership.isExpired()) {
            _logger.logger.info(`Ignoring expired device membership ${memberEvent.getSender()}/${membership.deviceId}`);
            continue;
          }
