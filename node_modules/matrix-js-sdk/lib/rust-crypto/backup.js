...
class RustBackupManager extends _typedEventEmitter.TypedEventEmitter {
  ...
  async handleBackupSecretReceived(secret) {
    ...
    if (!(backupCheck !== null && backupCheck !== void 0 && (_backupCheck$backupIn = backupCheck.backupInfo) !== null && _backupCheck$backupIn !== void 0 && _backupCheck$backupIn.version) || !backupCheck.trustInfo.trusted) {
      ...
      _logger.logger.warn("Received backup decryption key, but there is no trusted server-side key backup");
      return false;
    }
    try {
      ...
      if (!privateKeyMatches) {
        _logger.logger.debug(`onReceiveSecret: backup decryption key does not match current backup version`);
  ...
  async importBackedUpRoomKeys(keys, opts) {
    ...
    await this.olmMachine.importBackedUpRoomKeys(keysByRoom, (progress, total) => {
      ...
      while (!this.stopped) {
        ...
        try {
          ...
          try {
            ...
            this.emit(_crypto.CryptoEvent.KeyBackupSessionsRemaining, remaining);
          } catch (err) {
  ...
  async requestKeyBackupVersion() {
    try {
      return await this.http.authedRequest(_httpApi.Method.Get, "/room_keys/version", undefined, undefined, {
