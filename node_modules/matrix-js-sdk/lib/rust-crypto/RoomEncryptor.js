...
class RoomEncryptor {
  ...
  async prepareForEncryption(globalBlacklistUnverifiedDevices) {
    const logger = new _logger.LogSpan(this.prefixedLogger, "prepareForEncryption");
   ...
   * @param globalBlacklistUnverifiedDevices - When `true`, it will not send encrypted messages to unverified devices
   */
  async encryptEvent(event, globalBlacklistUnverifiedDevices) {
    var _event$getTxnId;
    const logger = new _logger.LogSpan(this.prefixedLogger, (_event$getTxnId = event.getTxnId()) !== null && _event$getTxnId !== void 0 ? _event$getTxnId : "");
    await this.ensureEncryptionSession(logger, globalBlacklistUnverifiedDevices);
